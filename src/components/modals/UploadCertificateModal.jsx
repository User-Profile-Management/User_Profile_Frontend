...
const UploadCertificateModal = ({ isOpen, onClose, onUpload }) => {
    ...
    const handleFileChange = (e) => {
        setCertificateData({ ...certificateData, certificateFile: e.target.files[0] });
    };

    const handleSubmit = (e) => {
        e.preventDefault();
        if (!certificateData.name || !certificateData.issuedBy || !certificateData.certificateFile) {
...
<label 
    ...
    <img src={CertificateIcon} alt="Document" className="w-5 h-5 mr-2" /> 
    {certificateData.certificateFile ? certificateData.certificateFile.name : "Choose file"}
</label>
    </div>
</div>
